...
describe('EventsService', () => {
  ...
  it('findAll() should return all events',async () => {
    expect(await service.findAll()).toMatchObject(eventResult)
  })

  it('findOne() should return one event',async () => {
    const id = new Schema.Types.ObjectId('65117aaea34fb2e66796de6e')
    expect(await service.findOne(id)).toMatchObject(eventResult)
  })
  ...
  it('update() should update an event',async () => {
    ...
    expect(await service.update(id, updateEventDto)).toMatchObject({
      ...eventResult,
      visibility: true
    })
  })
  ...
  // it('create() should return 400 status code',async () => {
  //   expect(await service.create(newEvent)).rejects.toThrowError(HttpException)
  // })
});
